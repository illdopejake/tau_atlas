...
def extract_roi_values(img_dict, atlas, otpt, img_names, roi_key = None,scale=None, comp = 'mean', method='nibabel'):
    ...
    for id,imgz in img_dict.iteritems():
        for img in imgz:
            for k,v in img_names.iteritems():
                if v in img:
                    itp = k
            if method == 'fsl':
                df = extract_values_from_img(img,atlas,df,id,itp,roi_key=roi_key,scale=scale,comp=comp)
            elif method == 'nibabel':
                df = extract_values_nibabel(img,atlas,df,id,itp,roi_key=roi_key,scale=scale,comp=comp)
            df.to_csv(os.path.join(wdir,'%s.xls'%(otpt)))
            print 'file written to %s'%(os.path.join(wdir,'%s.xls'%(otpt)))

    return df

def extract_values_from_img(img,atlas,df,id,itp,roi_key = None, scale = None,comp = 'mean'):
    ...
    if not roi_key:
        ...
        for i,roi in roi_key.iteritems():
            ...
            val = subprocess.check_output('fslstats %s_valz.nii.gz -M'%(cde),shell=True)
            os.system('rm %s_*'%(cde))
            df.ix[id, '%s_%s'%(itp,roi)] = float(val)

    return df

def extract_values_nibabel(img,atlas,df,id,itp,roi_key = None, scale = None, comp = 'mean'):
    ...
    if not roi_key and not scale:
        ...
        for i in unique:
            ...
            df.ix[id, '%s_%s'%(itp,i)] = np.mean(idata[msk.mask])
    ...
    if not roi_key and scale:
        ...
        for i in range(1,(scale+1)):
            ...
            msk = np.ma.masked_array(adata,adata==i)
            if comp == 'count':
                df.ix[id, '%s_%s'%(itp,i)] = np.mean(idata[msk.mask]) * len(idata[msk.mask])
            else:
                df.ix[id, '%s_%s'%(itp,i)] = np.mean(idata[msk.mask])
    ...
    if roi_key:
        ...
        for i,roi in roi_key.iteritems():
            print 'working on roi %s'%(roi)
            msk = np.ma.masked_array(adata,adata==i)
            df.ix[id, '%s_%s'%(itp,roi)] = np.mean(idata[msk.mask])
